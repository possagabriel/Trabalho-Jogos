...
from src.runtime.presentation.menu_scene import texto_espacado
from src.runtime.infrastructure.persistence.save_system import ARQUIVO_RECORDES
from src.runtime.infrastructure.persistence.shop import LojaSkins
from src.runtime.infrastructure.graphics.smooth import ease_out
...
class TelaContinuarJogo:
    ...
    def botoes(self) -> list[BotaoNeon]:
        ...
        return self.menu._botoes_rodape(
            ["CONTINUAR", "NOVO JOGO", "VOLTAR"])
    ...
    def desenhar(self, tela: pygame.Surface) -> None:
        ...
        menu._cabecalho_sub_animado(tela, "CONTINUAR JOGO", tema["primaria"])
        tem = self.tem_save()
        ...
        if tem:
            ...
            linhas = [
                ...
                ("Nivel", str(jogador["nivel_maximo"])),
                ("Pontos totais",
                 f"{jogador['total_pontos']:,}".replace(",", ".")),
                ("Chefes", str(jogador["bosses_derrotados"])),
                ("Moedas", f"{jogador['moedas']:,}".replace(",", ".")),
                ("Visual", skin.nome),
                ("Cenários", f"{len(jogador['cenarios_desbloqueados'])}/6"),
            ]
            y = painel.y + layout.px(62)
            for indice, (rotulo, valor) in enumerate(linhas):
                progresso = menu._frac_sub(0.20 + indice * 0.05, 0.35)
                dx, dy, alfa = menu._entrada_anim(
                    progresso, dx_design=-30)
                rotulo_surf = menu.fonte_media.render(
                    f"{rotulo}:", True, (170, 175, 225))
                menu._blit_alfa(
                    tela, rotulo_surf,
                    (painel.x + layout.px(70) + dx, y + dy),
                    int(255 * alfa))
                valor_surf = menu.fonte_media.render(valor, True, BRANCO)
                menu._blit_alfa(tela, valor_surf, valor_surf.get_rect(
                    midleft=(painel.x + layout.px(250) + dx,
                             y + layout.px(9) + dy)), int(255 * alfa))
                y += layout.px(36)
        else:
            dx, dy, alfa = menu._entrada_anim(
                menu._frac_sub(0.2, 0.35), dx_design=-20)
            superficie = menu.fonte_media.render(
